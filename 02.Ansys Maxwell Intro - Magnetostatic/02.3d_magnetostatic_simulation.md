### Step-by-Step Actions, Events, and Functions for 3D Magnetostatic Simulation of EMB Using Ansys Maxwell

#### **1. Launch and Load the Model**
- **Action:** Open Ansys Electronics Desktop (AEDT) Student Version.
- **Event:** Load the previously saved EMB model.
- **Function:** Ensures continuity from the geometry creation phase.

#### **2. Assign Boundary Conditions**
- **Action:** Expand the model tree, locate the "solids" folder, and select the "coil."
- **Event:** Right-click → Assign Boundary → Insulating.
- **Function:** Prevents current from flowing between conductors by acting as an insulating surface.

#### **3. Assign Excitation to the Coil**
- **Action:** 
  - Right-click the "coil" → Edit → Surface → Section.
  - In the pop-up, select the XZ plane → Click OK.
- **Event:** Creates a section in the coil to apply excitation.
- **Function:** Allows insertion of a current source.

#### **4. Define Excitation Properties**
- **Action:** 
  - Select the section under "sheets" → Right-click → Edit → Boolean → Separate Bodies.
  - Delete one of the two sections.
  - Assign excitation by selecting the section → Right-click → Assign Excitation → Current.
  - Set the excitation name as "Current1."
  - Assign variables "Amp" (unit type: Current, value: 1 A) and "Turns" (unit type: None, value: 200).
- **Event:** Defines excitation parameters.
- **Function:** Controls the current and turns of the coil.

#### **5. Assign Force Parameter to Brake Plate**
- **Action:** 
  - Select "brake plate" → Right-click → Assign Parameter → Force.
  - In the pop-up, set type to "Virtual."
- **Event:** Creates a virtual force calculation setup.
- **Function:** Computes force exerted by the electromagnet.

#### **6. Define the Simulation Region**
- **Action:** 
  - Click "Create Region" under the Draw tab.
  - In the pop-up, choose “Pad all directions similarly” → Enter value: 100.
- **Event:** Expands the simulation boundary.
- **Function:** Ensures accurate field calculation.

#### **7. Setup the Analysis**
- **Action:** 
  - Right-click "Analysis Setup" → Add Solution Setup.
  - Keep defaults → Click OK.
- **Event:** Defines the simulation analysis.
- **Function:** Prepares the model for parametric analysis.

#### **8. Define Parametric Setup**
- **Action:** 
  - Right-click "Optimetrics" → Add → Parametric.
  - Click "Add" in the pop-up.
  - Choose the "gap" variable and define a linear sweep (0.2 mm to 1 mm, step size: 0.1 mm).
  - Click "Add" → Click OK.
- **Event:** Defines parametric sweep for gap variation.
- **Function:** Analyzes how force changes with the air gap.

#### **9. Configure Mesh Settings**
- **Action:** 
  - Right-click "Mesh" → Select "Initial Mesh Settings."
  - Adjust "Curved Surface Meshing" to approximately 3.
- **Event:** Configures mesh resolution.
- **Function:** Ensures meshing is within the student version’s limit.

#### **10. Validate the Model**
- **Action:** Click "Validate" (green checkmark) in the Simulation tab.
- **Event:** Checks for errors in the setup.
- **Function:** Ensures the model is correctly configured.

#### **11. Run the Parametric Analysis**
- **Action:** 
  - Expand "Optimetrics" → Right-click "Parametric Setup" → Choose "Analyze."
- **Event:** Starts the simulation.
- **Function:** Runs the parametric sweep to compute force variations.

#### **12. Generate Force vs. Gap Plot**
- **Action:** 
  - Right-click "Results" → Create a Magnetostatic Report → Rectangular Plot.
  - Set solution to "Setup1: LastAdaptive."
  - Set parameter to "Force1."
  - Select quantity "Force_mag" → Click "New Report."
- **Event:** Generates a graph.
- **Function:** Shows force variation with air gap.

#### **13. Visualize Magnetic Flux Density**
- **Action:** 
  - Expand "Planes" → Select "Global:XZ Plane."
  - Right-click "Field Overlays" → Select "Fields > B > Mag_B."
  - In the pop-up, select "Mag_B" under "Quantity" → Click "Done."
- **Event:** Displays the magnetic flux density plot.
- **Function:** Provides visual insight into magnetic field distribution.

#### **14. Animate the Magnetic Field**
- **Action:** 
  - Right-click "Mag_B1" under "Field Overlays" → Select "Animate."
  - In the pop-up, choose "Swept Variables" → Select "ParametricSetup1."
- **Event:** Creates an animation of flux density variation.
- **Function:** Shows how magnetic field changes with air gap variation.

#### **15. Export Animation**
- **Action:** Adjust animation speed and export as a video file.
- **Event:** Saves the animated simulation.
- **Function:** Useful for presentations and analysis.

### **Conclusion**
This workflow completes the magnetostatic simulation of the electromagnetic brake (EMB) in Ansys Maxwell 3D. The simulation assesses how force varies with air gap changes, enabling further parametric analysis of turns and current variables. The next step involves converting the 3D model into a 2D simulation for comparison.    
